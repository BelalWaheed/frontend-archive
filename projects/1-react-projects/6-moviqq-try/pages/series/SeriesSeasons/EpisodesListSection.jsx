    ...
    <section className="max-w-6xl mx-auto px-6 pb-20">
      ...
      <div className="flex gap-4 scroll-indicator overflow-x-auto overflow-y-hidden pb-4">
        {seasonDetails?.episodes?.slice(0, 10).map((ep) => (
          <div
            onClick={() => {
              localStorage.setItem("episodeNumber", ep?.episode_number);
              dispatch(
          ...
          >
            ...
            <div className="p-3 space-y-2">
              <h3 className="text-lg font-medium truncate">{ep.name}</h3>
              <div className="flex items-center gap-2 text-gray-400 text-sm">
                <FaStar className="text-yellow-500" /> {ep.vote_average}
              </div>
              <div className="flex items-center gap-2 text-gray-400 text-sm">
                <FaCalendarAlt className="text-red-500" /> {ep.air_date}
              </div>
              <div className="flex items-center gap-2 text-gray-400 text-sm">
                <FaClock className="text-red-500" /> {ep.runtime} min
              </div>
            </div>
          </div>
        ))}
